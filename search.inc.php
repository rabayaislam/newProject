...
<?php
     ...
     if (isset($_POST['submit']))
    {
        ...
        {
            $sql = "SELECT roll_no, first_name, last_name, department, batch, phone, email FROM users WHERE department='$dep';";
            $result = mysqli_query($conn, $sql);
            $resultCheck = mysqli_num_rows($result);
            if ($resultCheck > 0)
            {
                ?>
      <br><h1 style="font-family: arial; font-size: 20px;color: #111;line-height: 50px; text-align: center;"><?php  echo $_POST['dep'];?>&nbspDepartment </h1><br>
